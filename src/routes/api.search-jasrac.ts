...
export const Route = createFileRoute("/api/search-jasrac")({
  server: {
    handlers: {
      ...
      GET: async ({ request }) => {
        try {
          ...
          if (!songsParam) {
            return new Response(JSON.stringify({ message: "検索する曲情報が指定されていません" }), {
              status: 400,
          ...
          return new Response(
            JSON.stringify({
              message: error instanceof Error ? error.message : "JASRAC情報の取得に失敗しました",
            }),
      ...
      POST: async ({ request }) => {
        try {
          ...
          if (!songs || !Array.isArray(songs) || songs.length === 0) {
            return new Response(JSON.stringify({ message: "曲情報が正しく指定されていません" }), {
              status: 400,
              headers: { "Content-Type": "application/json" },
            });
          }
