...
export const Route = createFileRoute("/api/jasrac-results")({
  server: {
    handlers: {
      // GETリクエスト - 検索結果を取得
      GET: async () => {
        try {
          ...
          if (!fs.existsSync(outputFile)) {
            ...
            return new Response(JSON.stringify({ message: "検索結果がまだありません" }), {
              status: 404,
          ...
          try {
            ...
            return new Response(JSON.stringify({ message: "検索結果の解析に失敗しました" }), {
              status: 500,
              headers: { "Content-Type": "application/json" },
            });
          }
          ...
          return new Response(
            JSON.stringify({
              message: error instanceof Error ? error.message : "検索結果の取得に失敗しました",
            }),
