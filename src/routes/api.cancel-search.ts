...
export const Route = createFileRoute("/api/cancel-search")({
  server: {
    handlers: {
      POST: async () => {
        try {
          // プロセスIDファイルのパス（server/jasrac-bridge.ts と同じディレクトリを使用）
          ...
          return new Response(
            JSON.stringify({
              message:
                error instanceof Error ? error.message : "検索キャンセル中にエラーが発生しました",
            }),
