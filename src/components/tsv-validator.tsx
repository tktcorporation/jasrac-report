...
export function TsvValidator({ results }: TsvValidatorProps) {
		...
		data.forEach((row, rowIndex) => {
			...
			Object.entries(row).forEach(([column, value]) => {
				...
				if (rule.conditionallyRequired && value === "") {
					...
					// 依存項目の値によって必須の項目
					if (
						rule.dependsOnValues &&
						dependsOn &&
						rule.dependsOnValues.includes(dependsOnValue)
					) {
						errors.push({
							row: rowIndex + 1,
							column: column,
	...
	const validateFilename = (): boolean => {
		...
		if (licenseNumber || useYearMonth) {
			...
			if (
				licenseNumber &&
				(licenseNumber.length !== 10 || !/^[a-zA-Z0-9]+$/.test(licenseNumber))
			) {
			...
			if (
				useYearMonth &&
				(useYearMonth.length !== 6 || !/^\d{6}$/.test(useYearMonth))
			) {
				setFilenameError(
					"利用年月はYYYYMM形式で入力してください（例：202404）",
				);
				return false;
			}
														...
														<TooltipContent className="max-w-sm">
															<div className="space-y-2">
																...
																{columnRules[header].validValues && (
																	<p className="text-sm text-blue-500">
																		有効値:{" "}
																		{columnRules[header].validValues.join(", ")}
																	</p>
																)}
