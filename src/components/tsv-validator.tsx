import { AlertCircle, CheckCircle, Download, Edit, HelpCircle, Info, XCircle } from "lucide-react";
...
export function TsvValidator({ results }: TsvValidatorProps) {
  const [filename, setFilename] = useState<string>(
    `jasrac_report_${new Date().toISOString().split("T")[0]}`,
  );
  ...
  const validateFilename = (): boolean => {
    ...
    if (licenseNumber || useYearMonth) {
      ...
      if (
        additionalText &&
        (additionalText.length > 100 || !/^[a-zA-Z0-9]*$/.test(additionalText))
      ) {
  ...
  const exportTsv = () => {
    ...
    if (dataValidationErrors.length > 5) {
      if (
        !window.confirm(
          `${dataValidationErrors.length}件の検証エラーがあります。このままエクスポートしますか？`,
        )
      ) {
        return;
      }
  ...
  return (
    <div className="space-y-6">
      {isValid !== null && (
        <Alert
          className={isValid ? (hasDataWarnings ? "bg-yellow-50" : "bg-green-50") : "bg-red-50"}
        >
          {isValid ? (
            hasDataWarnings ? (
          ...
          <AlertTitle>
            {isValid ? (hasDataWarnings ? "検証完了（警告あり）" : "検証成功") : "検証失敗"}
          </AlertTitle>
          <AlertDescription>
            {isValid
              ? hasDataWarnings
        ...
        <CardContent className="p-0 sm:p-0">
            ...
            <Table>
                              ...
                              <div className="space-y-2">
                                ...
                                {columnRules[header].fixedValue && (
                                  <p className="text-sm text-blue-500">
                                    固定値: {columnRules[header].fixedValue}
                                  </p>
                                )}
                                {columnRules[header].validValues && (
                                  <p className="text-sm text-blue-500">
                                    有効値: {columnRules[header].validValues.join(", ")}
                                  </p>
                                )}
              ...
              <TableBody>
                {tsvData.slice(0, showRowCount).map((row, rowIndex) => (
                  <TableRow key={`row-${rowIndex}-${row.インターフェイスキーコード || rowIndex}`}>
                      ...
                      return (
                        ...
                        >
                          {isEditable ? (
                            ...
                            <div
                              className={`text-sm px-2 py-1 rounded ${isJasracField ? "bg-blue-50" : "bg-gray-50"}`}
                            >
          ...
          {tsvData.length > showRowCount && (
            <div className="mt-4 text-center pb-4">
              <Button variant="outline" onClick={handleShowMoreRows}>
      ...
      {dataValidationErrors.length > 0 && (
        <Card className="border-yellow-300">
          <CardHeader>
            ...
            <CardDescription>以下のデータ項目に問題があります</CardDescription>
          </CardHeader>
          <CardContent>
              ...
              <Table>
                ...
                <TableBody>
                  {dataValidationErrors.map((error, index) => (
                    <TableRow key={`error-${index}-${error.row}-${error.column}`}>
      ...
      <Card>
        ...
        <CardContent>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4">
            ...
            <div>
              <Label htmlFor="use-year-month" className="block text-sm font-medium mb-1">
                利用年月（YYYYMM）
              </Label>
              <Input
                id="use-year-month"
            ...
            <div>
              <Label htmlFor="additional-text" className="block text-sm font-medium mb-1">
                追加テキスト（任意、英数字）
              </Label>
              <Input
                id="additional-text"
          ...
          {filenameError && <p className="text-red-500 text-sm mb-4">{filenameError}</p>}
          <div className="flex justify-center mt-4">
            ...
            <a ref={downloadLinkRef} className="hidden" href="./" download="jasrac-data.txt">
              ダウンロード
            </a>
          </div>
        </CardContent>
        <CardFooter className="text-sm text-gray-500">
          <div className="space-y-2 w-full">
            <p>
              ファイル名は「許諾番号 + 利用年月 + 追加テキスト.txt」の形式で保存されます。
              <br />
              例: J240841199202404A01.txt
            </p>
            <div>
              <h3 className="font-medium">全体注記:</h3>
              <ul className="list-disc pl-5 space-y-1">
                {generalNotes.map((note, index) => (
                  <li key={`note-${index}-${note.note.substring(0, 10).replace(/\s+/g, "")}`}>
                    {note.note}
                  </li>
                ))}
              </ul>
