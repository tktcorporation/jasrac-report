...
export function PlaywrightLogsPanel({
  ...
  const isProcessCompleted = useCallback(() => {
    ...
    return lastLogs.some((log) => completionKeywords.some((keyword) => log.includes(keyword)));
  }, [logs]);
      ...
      <Collapsible open={isOpen} onOpenChange={setIsOpen}>
        <div className="flex justify-between items-center mb-2">
          <div className="flex items-center gap-2">
            ...
            {isPolling && (
              <span className="text-xs bg-blue-600 px-2 py-0.5 rounded-full ml-2">自動更新中</span>
            )}
            {isProcessCompleted() && (
              <span className="text-xs bg-green-600 px-2 py-0.5 rounded-full ml-2">完了</span>
            )}
          </div>
          <div className="flex items-center gap-2">
            {isPolling && (
        ...
        <CollapsibleContent>
          <div className="h-48 overflow-y-auto p-2 bg-slate-950 rounded" ref={logsContainerRef}>
            {logs.length > 0 ? (
