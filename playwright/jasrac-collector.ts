...
async function performSearchWithFallback(page: Page, song: SongInfo): Promise<JasracInfo | null> {
  ...
  if (song.lyricist && song.composer) {
    availableStrategies.push(async () => {
      console.log(
        `戦略: タイトル(${song.title}) + 作詞者(${song.lyricist}) + 作曲者(${song.composer})${song.artist ? " + アーティスト(" + song.artist + ")" : ""}で検索`,
      );
...
async function processDetailPage(newPage: Page, song: SongInfo): Promise<JasracInfo | null> {
  try {
    ...
    try {
      // 作品コード
      const workCode = await newPage.$eval(
        "div.baseinfo--code div.detail_iPhone_link > strong",
...
async function extractUsagePermissions(page: Page): Promise<UsagePermission> {
  ...
  try {
    ...
    for (const tabId of tabIds) {
      ...
      if (tabExists) {
        // タブの中のテキストを取得
        const tabContent = await page.$eval(
          `#${tabId} dl.consent dd.txt p`,
          (el: Element) => el.textContent?.trim() || "",
        );
        ...
        // 管理詳細情報を取得
        const managementDetailText = await page.$$eval(
          `#${tabId} table.detail tr`,
          (rows: Element[]) => {
...
async function writeTsvFile(songs: JasracInfo[], outputPath: string) {
  ...
  // 拡張ヘッダーを追加
  const header =
    "インターフェイスキーコード\tコンテンツ区分\tコンテンツ枝番\tメドレー区分\tメドレー枝番\tコレクトコード\tＪＡＳＲＡＣ作品コード\t原題名\t副題・邦題\t作詞者名\t補作詞・訳詞者名\t作曲者名\t編曲者名\tアーティスト名\t情報料（税抜）\tＩＶＴ区分\t原詞訳詞区分\tIL区分\tリクエスト回数\t演奏時間\t作品種別\t国籍区分\t作成年月日\t利用分野\t出版社\t権利者情報\n";
  ...
  const detailedHeader =
    "作品コード\tタイトル\t作詞者\t作曲者\t編曲者\tアーティスト\t演奏時間\t作品種別\t国籍区分\t作成年月日\t利用分野\t出版社\t利用許可情報\t権利者情報\n";
  ...
  songs.forEach((song) => {
    ...
    // 利用許可情報を文字列に変換
    const usagePermissionStr = song.usagePermissions
      ? formatUsagePermissions(song.usagePermissions)
      : "";
